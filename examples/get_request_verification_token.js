...
const getRequestVerificationToken = async () => {
  const uri = new URL('Investor/#Home', baseUri).toString()
  ...
  const page2 = await agent.get('/Investor/mobile/tomobile', {
    redirect: 'manual',
    headers: {
      'Sec-Fetch-Dest': 'document',
      'Sec-Fetch-Mode': 'navigate',
  ...
  return
  const page3 = await agent.get('http://www-us.computershare.com/Investor/', {
    redirect: 'manual',
    headers: {
      'Sec-Fetch-Dest': 'document',
      'Sec-Fetch-Mode': 'navigate',
