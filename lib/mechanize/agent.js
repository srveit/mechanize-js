...
function logPage(agent, body, options, response, fn) {
    ...
    filename =
        ...
        (ext ? '.' + ext : '');
    encoding = 'utf8';
    fs.writeFile(filename, body, 0, encoding, function (err) {       // eslint-disable-line no-unused-vars
...
function addResponseCookie(cookieJar, cookieString, domain, path) {
    var cookieStringSplitter = /[:](?=\s*[a-zA-Z0-9_\-]+\s*[=])/g;       // eslint-disable-line no-useless-escape
    cookieString.split(cookieStringSplitter).forEach(function (cookieStr) {
        var cookie, successful;       // eslint-disable-line no-unused-vars
...
function fetchPage(agent, options, fn) {
    ...
    agent.request(options, function (err, response, body) {
        ...
        if (agent.logPages && agent.logDir) {
            logPage(agent, body, options, response, function (err, filename) {       // eslint-disable-line no-unused-vars
...
Agent.prototype.submit = function (form, button, headers, requestOptions, fn) {
    ...
    if (form.method.match(/^post$/i)) {
        ...
        var actionUrl = form.action.replace(/\?[!#$&-;=?-\[\]_a-z~]*$/, '');       // eslint-disable-line no-useless-escape
