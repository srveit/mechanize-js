...
describe('Mechanize/Agent', () => {
  ...
  describe('getting page', () => {
    ...
    describe('with header cookies', () => {
      ...
      beforeEach(async () => {
        ...
        await agent.get(baseUrl + loginPath, { debug: true })
      })
      ...
      it('should set cookies', async () => {
        ...
        console.log('domain', domain, 'cookies', cookies)
        expect(cookies.length).toBe(3)
      })

      describe('and getting redirected page', () => {
        const redirectURI = '/Investor/mobile/tomobile'
        beforeEach(async () => {
          server.getPage.mockClear()
          await agent.get(redirectURI, {
            redirect: 'manual',
            headers: {
        ...
        it('should call getPage with cookies', async () => {
          expect(server.getPage).toBeCalledWith(
            expect.objectContaining({
              path: redirectURI,
            })
      ...
      it('should get page with referrer', async () => {
        expect(server.getPage).toHaveBeenCalledWith({
          ...
          headers: {
            ...
            'user-agent': `Mechanize/1.0.0 Node.js/${process.version} (http://github.com/srveit/mechanize-js/)`,
          },
          path: '/page.html',
          query: {},
        })
  ...
  describe('getting page with form', () => {
    ...
    describe('then submitting form', () => {
      beforeEach(async () => {
        await agent.setCookie(`sessionid=1234;domain=${domain};path=/`, url)
        await agent.setCookie('name=bob', url)
        await agent.submit({
          form,
        })
      })
  ...
  describe('getting page with form with amp value', () => {
    let url, form
    beforeEach(async () => {
      url = baseUrl + '/page.html'
      const responseBody = await fixture('form_with_amp.html')
      server.getPage.mockReturnValueOnce(responseBody)
      const page = await agent.get(url)
    ...
    describe('then submitting form', () => {
      ...
      it('should post the form', () => {
        expect(server.postForm).toHaveBeenCalledWith({
          ...
          query: {},
          body: {
            field2: '<Response Context="rm=0&amp;id=passive">',
          },
        })
      })
    })
