...
const evaluate = (xpathExpression, contextNode, resultType) => {
  const document =
    contextNode.constructor.name === 'Document'
      ? contextNode
      : contextNode.ownerDocument
  return document.evaluate(xpathExpression, contextNode, null, resultType, null)
}

const at = (doc, xpathExpression) =>
  evaluate(xpathExpression, doc, XPathResult.FIRST_ORDERED_NODE_TYPE)
    .singleNodeValue

const search = (doc, xpathExpression) =>
  xPathResultToArray(
    evaluate(xpathExpression, doc, XPathResult.ORDERED_NODE_ITERATOR_TYPE)
  )

const textContent = (element) => element && element.textContent

export { at, nodeAttr, parseHtmlString, search, textContent }
